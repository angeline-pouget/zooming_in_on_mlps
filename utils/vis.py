...
def feature_inversion(model, 
    ...
    plt.title(f'Feature inversion for model {tit}')
    for i in range(len(recreated_imgs)):
        if rgb:
            axs.imshow(postprocess_image(recreated_imgs[i], rgb = rgb))
        else:
            axs.imshow(postprocess_image(recreated_imgs[i], rgb = rgb), cmap = 'gray')
